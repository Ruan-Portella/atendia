 ...
 * domínio de outra.
 */
export const hostAgency = cache(async (): Promise<{ id: string; name: string; logo_url: string | null; brand_color: string; support_whatsapp: string | null } | null | undefined> => {
  const host = (await headers()).get("host");
  if (!isCustomHost(host)) return undefined;
  const domain = host!.toLowerCase().split(":")[0];
  const { data } = await createAdminClient()
    .from("agencies")
    .select("id, name, logo_url, brand_color, support_whatsapp")
