...
import { change, currentPeriodBR, getClientReport, isPeriod, periodLabel, periodRange, shiftPeriod } from "@/lib/report";
...
export default async function ClientPortalPage({ params, searchParams }: PageProps<"/c/[token]">) {
  ...
  const color = /^#[0-9a-f]{6}$/i.test(report.agency.brand_color) ? report.agency.brand_color : "#1f4e3d";
  const month = periodLabel(period);
  const wa = report.agency.support_whatsapp ? `https://wa.me/${report.agency.support_whatsapp.replace(/\D/g, "")}` : null;
        ...
        <div className="mx-auto flex max-w-[980px] flex-wrap items-center justify-between gap-3 px-4 py-4 sm:px-6">
          <div className="flex min-w-0 items-center gap-2.5">
            {report.agency.logo_url ? (
              ...
              <img src={report.agency.logo_url} alt="" className="h-8 w-8 rounded-lg object-contain" />
            ) : (
              <span className="display flex h-8 w-8 items-center justify-center rounded-lg text-[13px] font-bold text-white" style={{ background: color }}>{initials(report.agency.name)}</span>
            )}
            <span className="display truncate text-base font-bold">{report.agency.name}</span>
          </div>
      ...
      <main className="mx-auto flex max-w-[980px] flex-col gap-6 px-4 py-6 sm:px-6 sm:py-8">
        <div className="flex flex-wrap items-end justify-between gap-3">
          ...
          <nav aria-label="Mês do relatório" className="flex items-center gap-1 rounded-xl border border-line bg-panel p-1 print:hidden">
            <Link href={`/c/${token}?mes=${shiftPeriod(period, -1)}`} className="btn-icon" aria-label="Mês anterior"><ChevronLeft size={16} /></Link>
            <span className="min-w-[130px] text-center text-sm font-semibold capitalize">{month}</span>
            {period < today ? (
              <Link href={`/c/${token}?mes=${shiftPeriod(period, 1)}`} className="btn-icon" aria-label="Próximo mês"><ChevronRight size={16} /></Link>
            ) : (
        ...
        <p className="text-[15px] text-ink-2">
          {c.conversations
            ? <>Em {month}{period === today ? " (até agora)" : ""}, o assistente atendeu <strong className="text-ink">{num(c.conversations)} pessoa{c.conversations === 1 ? "" : "s"}</strong> e capturou <strong className="text-ink">{num(c.leads)} contato{c.leads === 1 ? "" : "s"}</strong>{c.conversations ? `, resolvendo ${report.resolvedPct}% das conversas sozinho` : ""}.</>
            ...
            {(leads ?? []).map((l) => (
              <div key={l.id} className="flex flex-col gap-1 border-b border-line-2 px-4 py-3 text-sm last:border-0 sm:grid sm:grid-cols-[1fr_1.4fr_2fr_auto] sm:items-center sm:gap-3">
                ...
                <span className="flex items-center gap-3 text-xs text-muted">
                  ...
                  {l.conversation_id && <Link href={`/c/${token}/conversas/${l.conversation_id}`} className="font-semibold hover:underline print:hidden">Conversa</Link>}
                </span>
              </div>
            ))}
          </div>
        ...
        <section className="flex flex-col gap-3 print:hidden">
          ...
          <div className="card overflow-hidden">
            ...
            {(conversations ?? []).map((cv) => (
              <Link key={cv.id} href={`/c/${token}/conversas/${cv.id}`} className="flex flex-wrap items-center gap-x-3 gap-y-1 border-b border-line-2 px-4 py-3 text-sm last:border-0 hover:bg-ground">
